...
bool ishexnstring(const QString& string) {
  for (int i = 0; i < string.length(); i++) {
    if (isxdigit(string[i] == 0)) return false;
  }
...
int JSonScanner::yylex(YYSTYPE *yylval, yy::location *yylloc) {
  ...
  else if (m_allowSpecialNumbers && m_quotmarkClosed &&
           ((ch == 'I') || (ch == 'i'))) {
    QByteArray ret(1, ch);
  ...
  if ((ch != '"') && !m_quotmarkClosed) {
    ...
    while (true) {
      ...
      if (ret != 1) {
        if (m_io->atEnd()) return yy::json_parser::token::END;
        else return -1;
      }
      ...
      else escape_on = (prevCh == '\\') ? true : false;
